...
def test_load_metrics_tracking(registry):
    ...
    # Register a test model
    registry.register("test-model", ModelType.GENERIC, MockModel, MockConfig)
...
def test_lru_cache_eviction(registry):
    ...
    for i in range(3):
        name = f"model-{i}"
        registry.register(name, ModelType.GENERIC, MockModel, MockConfig)
    ...
    registry.register("model-3", ModelType.GENERIC, MockModel, MockConfig)
