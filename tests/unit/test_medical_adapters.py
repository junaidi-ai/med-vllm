...
class SimpleModel(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        if input_ids is not None:
            ...
            input_ids = torch.zeros((batch_size, seq_length), dtype=torch.long, device=self.device)
            embedding_output = self.embeddings(input_ids)
        ...
        encoder_outputs = self.encoder(
            embedding_output,
            src_key_padding_mask=attention_mask.squeeze(1).squeeze(1) if attention_mask is not None else None
        )
...
class SimpleTokenizer:
    ...
    def __call__(self, text, *args, **kwargs):
        ...
        words = text.split()
        input_ids = [self.cls_token_id] + [hash(word) % (self.vocab_size - 5) + 5 for word in words]
        ...
        class MockBioBERTAdapter:
            def __init__(self, model, config=None):
                ...
                self.biomedical_terms = [
                    ...
                    'tachycardia', 'bradycardia', 'pneumonia', 'asthma'
                ]
    ...
    def setUp(self):
        ...
        class MockClinicalBERTAdapter:
            def __init__(self, model, config=None):
                ...
                self.clinical_terms = [
                    'admission', 'discharge', 'symptoms', 'diagnosis',
                    'treatment', 'medication', 'allergies', 'prognosis'
                ]
