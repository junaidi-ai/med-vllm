...
sys.modules['transformers.tokenization_utils_base'] = types.ModuleType('transformers.tokenization_utils_base')
...
def test_adapter_forward(mock_device_props, mock_cuda_available):
    ...
    with patch.object(adapter, '_forward_with_cache', return_value=mock_output[0]) as mock_forward_cache:
        adapter.kv_cache = {}
        output = adapter(input_ids, attention_mask=attention_mask)
        assert torch.equal(output, mock_output[0])
