...
def patch_adapters():
    ...
    for adapter_cls in [MockBioBERTAdapter, MockClinicalBERTAdapter, BioBERTAdapter, ClinicalBERTAdapter]:
        if not hasattr(adapter_cls, 'from_pretrained'):
            adapter_cls.from_pretrained = classmethod(
                lambda cls, model_name_or_path, **kwargs: cls(config={"model_type": cls.__name__.lower()})
            )
...
def patch_transformers():
    ...
    def patched_post_init(self):
        ...
        if not hasattr(self, 'hf_config'):
            ...
            self.hf_config.model_type = 'medical_llm' if hasattr(self, 'model_type') and self.model_type == 'medical_llm' else 'bert'
    ...
    from medvllm.medical.config.base import BaseMedicalConfig as OriginalBaseMedicalConfig
...
def patch_medical_config():
    ...
    def convert_enum_to_str(value):
        ...
        if isinstance(value, str):
            ...
            if value.lower() in ['disease', 'symptom', 'treatment', 'medication', 'procedure', 'anatomy']:
                return value.lower()
            return value.lower()
    ...
    def patched_init(self, *args, **kwargs):
        ...
        defaults = {
            ...
            'section_headers': [
                'history_of_present_illness', 'past_medical_history', 'medications',
                'allergies', 'family_history', 'social_history', 'review_of_systems',
                'physical_exam', 'assessment_and_plan'
            ],
        ...
        for key, default in defaults.items():
            if key not in kwargs and key not in ('medical_specialties', 'anatomical_regions'):
                kwargs[key] = default
    ...
    def patched_from_dict(cls, config_dict, **kwargs):
        ...
        for field in ['medical_entity_types', 'medical_specialties', 'anatomical_regions', 'imaging_modalities']:
    ...
    def patched_to_dict(self):
            ...
            if value is None:
                ...
                for item in value:
                    if hasattr(item, 'value'):  # Handle enum items in lists
                        ...
                        processed.append({k: str(v.value).lower() if hasattr(v, 'value') else v 
                                       for k, v in item.items()})
                    else:
                        processed.append(str(item).lower() if item is not None else None)
                result[field] = [p for p in processed if p is not None]
            elif isinstance(value, dict):
                result[field] = {k: str(v.value).lower() if hasattr(v, 'value') else v 
        ...
        for field, field_info in enum_fields.items():
            ...
            for item in result[field]:
                ...
                elif isinstance(item, str) and '.' in item and field_info['enum_type'] and field_info['enum_type'].lower() in item.lower():
                    value = item.split('.')[-1]
                    if field_info.get('force_lower', False):
                        value = value.lower()
                    processed_values.append(value)
    ...
    def patched_copy(self, update=None, **kwargs):
        ...
        for key, value in updates.items():
            if not hasattr(new_config, key):
                raise AttributeError(f"'{new_config.__class__.__name__}' object has no attribute '{key}'")
    ...
    for attr, default in default_attrs.items():
        if not hasattr(MedicalModelConfig, attr):
            setattr(MedicalModelConfig, attr, property(
...
def apply_patches():
    """Apply all patches before each test."""
    ...
    # Apply patches for specific modules
    with patch.multiple(
        'medvllm.models.adapters.base.MedicalModelAdapterBase',
        from_pretrained=classmethod(lambda cls, *args, **kwargs: cls()),
        setup_for_inference=MagicMock(return_value=None)
    ), patch.multiple(
        'medvllm.models.medical_model.MedicalModel',
        from_pretrained=classmethod(lambda cls, *args, **kwargs: cls()),
        generate=MagicMock(return_value=mock_tensor)
    ):
