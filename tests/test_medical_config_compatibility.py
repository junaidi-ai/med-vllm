...
sys.modules["flash_attn.flash_attn_interface"].flash_attn_with_kvcache = (
    lambda *args, **kwargs: None
)
...
class TestMedicalConfigCompatibility(unittest.TestCase):
    ...
    def test_base_config_compatibility(self, mock_from_pretrained):
        ...
        self.assertEqual(
            config.max_num_batched_tokens, base_params["max_num_batched_tokens"]
        )
        ...
        self.assertEqual(
            config.gpu_memory_utilization, base_params["gpu_memory_utilization"]
        )
        self.assertEqual(
            config.tensor_parallel_size, base_params["tensor_parallel_size"]
        )
    ...
    def test_medical_config_serialization(self, mock_from_pretrained):
        ...
        self.assertEqual(
            new_config.model_type, "bert"
        )  # Should be 'bert' not 'biobert'
        self.assertEqual(
            new_config.max_medical_seq_length,
            self.sample_config["max_medical_seq_length"],
        )
        self.assertEqual(
            new_config.medical_specialties, self.sample_config["medical_specialties"]
        )
        self.assertEqual(
            new_config.anatomical_regions, self.sample_config["anatomical_regions"]
        )
        self.assertEqual(
            new_config.enable_uncertainty_estimation,
            self.sample_config["enable_uncertainty_estimation"],
        )
    ...
    def test_json_serialization(self, mock_from_pretrained):
        ...
        self.assertEqual(
            loaded_config.max_medical_seq_length, test_config["max_medical_seq_length"]
        )
        self.assertEqual(
            loaded_config.medical_specialties, test_config["medical_specialties"]
        )
        self.assertEqual(
            loaded_config.anatomical_regions, test_config["anatomical_regions"]
        )
        self.assertEqual(
            loaded_config.enable_uncertainty_estimation,
            test_config["enable_uncertainty_estimation"],
        )

        # Verify the same for string-loaded config
        self.assertEqual(loaded_from_str.model_type, "bert")
        self.assertEqual(
            loaded_from_str.max_medical_seq_length,
            test_config["max_medical_seq_length"],
        )
        self.assertEqual(
            loaded_from_str.medical_specialties, test_config["medical_specialties"]
        )
        self.assertEqual(
            loaded_from_str.anatomical_regions, test_config["anatomical_regions"]
        )
        self.assertEqual(
            loaded_from_str.enable_uncertainty_estimation,
            test_config["enable_uncertainty_estimation"],
        )
    ...
    def test_backward_compatibility(self, mock_from_pretrained):
        ...
        with self.assertRaises(ValueError):
            with patch(
                ...
                MedicalModelConfig(
                    **{**base_params, "medical_specialties": "not_a_list"}
                )
        ...
        with self.assertRaises(ValueError) as cm:
            with patch(
                ...
                side_effect=ValueError("Invalid model path"),
            ):
        ...
        with self.assertRaises(AssertionError):
            with patch(
                ...
                MedicalModelConfig(
                    **{**base_params, "tensor_parallel_size": 10}
                )  # Max is 8
