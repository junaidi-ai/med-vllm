...
from tests.unit.engine.test_registry_metadata_caching import (
    MockConfig,
    MockModel,
    ModelRegistry,
    ModelType,
...
def test_model_loading_performance(registry):
    ...
    registry.register("perf-test-model", ModelType.GENERIC, MockModel, MockConfig)
...
def test_cache_hit_performance(registry):
    ...
    print(
        f"Cache hit: {hit_duration*1000:.3f}ms, Cache miss: {miss_duration*1000:.3f}ms"
    )
...
def test_concurrent_load_performance(registry):
    ...
    for i in range(num_models):
        registry.register(f"concurrent-{i}", ModelType.GENERIC, MockModel, MockConfig)
    ...
    for i in range(num_models):
        t = threading.Thread(target=load_models, args=(f"concurrent-{i}", results))
        threads.append(t)
        t.start()
    ...
    print(
        f"Sequential: {seq_duration*1000:.1f}ms, Concurrent: {conc_duration*1000:.1f}ms"
    )
...
def test_memory_usage(registry):
    ...
    num_models = 20
    for i in range(num_models):
        registry.register(f"memory-test-{i}", ModelType.GENERIC, MockModel, MockConfig)
...
def test_cache_eviction_performance(registry):
    ...
    for i in range(num_models):
        registry.register(
            f"eviction-test-{i}", ModelType.GENERIC, MockModel, MockConfig
        )
    ...
    print(
        f"Cache hit: {cache_hit_duration*1e6:.1f}µs, Eviction: {eviction_duration*1e6:.1f}µs"
    )
