...
class TestModelRegistryPerformance:
    ...
    def test_cache_efficiency(self):
        ...
        # Use a small number of test models to keep the test fast
        test_models = [
            f"test-model-{i}" for i in range(5)
                ...
                if cache_size < len(test_models):
                    ...
                    for _ in range(NUM_ITERATIONS):
                        self.registry.load_model(
                            test_models[cache_size], use_cache=True
                        )
                    cache_miss_time = (
        ...
        assert any(
            hit_time > 0 for hit_time, _ in results.values()
    ...
    def test_concurrent_loading(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            start_time = time.perf_counter()
            futures = [
                executor.submit(load_model_safely, name) for name in test_model_names
            ]
