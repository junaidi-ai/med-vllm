...
class ConfigVersioner:
    ...
    def check_version_compatibility(cls, version: str) -> None:
        ...
        try:
            ...
            if version_info.status == ConfigVersionStatus.UNSUPPORTED:
                raise ValueError(
                    f"Version {version} is no longer supported. " f"{version_info.message}"
                )
            ...
            if version != cls.CURRENT_VERSION:
                raise ValueError(
                    f"Unsupported version: {version}. " f"Current version is {cls.CURRENT_VERSION}"
                ) from e
            raise
    ...
    def migrate_config(cls, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if version not in cls.VERSIONS:
            raise ValueError(f"Cannot migrate from unsupported version: {version}")
