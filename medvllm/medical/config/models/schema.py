...
class MedicalModelConfigSchema(BaseModel):
    ...
    medical_entity_types: List[str] = Field(
        default_factory=list,
        description="Types of medical entities to recognize"
    )
    ...
    max_entity_span_length: int = Field(
        default=10,
        description="Maximum token length for entity spans",
        gt=0
    )
    ...
    document_types: List[str] = Field(
        default_factory=list,
        description="Types of clinical documents supported"
    )
    
    section_headers: List[str] = Field(
        default_factory=list,
        description="Common section headers in clinical documents"
    )
    ...
    domain_adaptation: bool = Field(
        default=False,
        description="Whether to enable domain adaptation"
    )
    ...
    regulatory_compliance: List[str] = Field(
        default_factory=list,
        description="Regulatory standards the model complies with"
    )
